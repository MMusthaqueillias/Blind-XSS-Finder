...
function scan() {
    ...
    var custom_payload =$(".custom_payload").val();
 ...
 if(scan_name==""){
    status=false;
    Message("please enter a scan name", 'Error', 'error');
 }
 if(url_file==""){
    status=false;
...
if (!txtExtensionRegex.test(custom_payload) && custom_payload!="") {
   
    status=false;
    Message("Please enter the custom_payload path of a .txt file", 'Error', 'error');
